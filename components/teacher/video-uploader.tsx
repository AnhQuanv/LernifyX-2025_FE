...
export function VideoUploader({
  ...
  // 2. Logic Upload chính (Tách ra để dùng chung)
  ...
  const handleVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isPaused && savedFileName && file.name !== savedFileName) {
      setPendingFile(file);
      setIsChangeFileConfirmOpen(true);
      return;
    }
  ...
  return (
    <>
              ...
              {uploading ? (
                ...
                <div className="flex flex-col items-center text-center space-y-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <span className="px-4 py-2 bg-black text-white text-[11px] font-bold rounded-lg uppercase">
                      Chọn file ban đầu để tiếp tục
                    </span>
                    <button
                      onClick={handleCancelProgress}
                      className="flex items-center justify-center gap-1 text-xs text-slate-500 hover:text-red-600 transition-colors cursor-pointer"
                    >
                      Hủy bỏ tiến trình này
                    </button>
                  </div>
                </div>
              ) : (
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="text-black-600 flex items-center gap-2">
              Xác nhận hủy bỏ?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Hành động này sẽ xóa toàn bộ tiến trình tải lên của file{" "}
              <strong>{savedFileName}</strong>. Bạn sẽ cần phải tải lại từ đầu
              nếu muốn.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Video bài học này sẽ bị xóa vĩnh viễn. Hành động này không thể
              hoàn tác.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      {/* 3. DIALOG CẢNH BÁO ĐỔI FILE KHÁC */}
      <AlertDialog
        open={isChangeFileConfirmOpen}
        onOpenChange={setIsChangeFileConfirmOpen}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Tải lên file mới?</AlertDialogTitle>
            <AlertDialogDescription>
              File bạn vừa chọn khác với file đang tạm dừng (
              <strong>{savedFileName}</strong>). Bạn có muốn hủy tiến trình cũ
              để tải file mới này từ đầu?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
